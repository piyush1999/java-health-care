...
public class ViewDoctorFrame extends javax.swing.JFrame {
    ...
    private void showDataInTable() {
        try {
            ArrayList<DoctorListPojo> doctorList = DoctorDao.getAllDoctor();
            //System.out.println("arraylist"+employeesList);
            if (doctorList.isEmpty()) {
                JOptionPane.showMessageDialog(null, "Sorry!No Doctor Are Found!", "Error", JOptionPane.INFORMATION_MESSAGE);
                return;
            }
                Object[] rows = new Object[8];
                DefaultTableModel model = (DefaultTableModel) jtDoctorDetails.getModel();
                for (DoctorListPojo doctor : doctorList) {
                    rows[0] = doctor.getEmpId();
                    rows[1] = doctor.getEmpName();
                    rows[2] = doctor.getSal();
                    rows[3] = doctor.getUserid();
                    rows[4] = doctor.getDoctorid();
                    rows[5] = doctor.getQualification();
                    rows[6] = doctor.getSpecialist();
                    model.addRow(rows);
                }
    ...
    private void initComponents() {
        ...
        jtDoctorDetails.setBackground(java.awt.Color.white);
        jtDoctorDetails.setFont(new java.awt.Font("Ubuntu", 1, 16)); // NOI18N
        jtDoctorDetails.setModel(new javax.swing.table.DefaultTableModel(
            new Object [][] {

            },
            new String [] {
                "EMP-ID", "EMP-NAME", "SALARY", "USER-ID", "DOCTOR-ID", "QUALIFICATION", "SPECAILIST"
            }
        ) {
        ...
        jScrollPane1.setViewportView(jtDoctorDetails);
